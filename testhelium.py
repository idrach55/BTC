...
class MyTest(unittest.TestCase):
	def test_update_bids(self):
		class Demo(Helium):
			def onPlace(self, oid, side, price, size):
				assert oid  == "B1"
				assert side == "buy"
				assert price == 0.5*(100.00 + 100.10) - 0.05
				assert size == 1.0
	...
	def test_onPartialFill(self):
		class Demo(Helium):
			...
			def onPlace(self, oid, side, price, size):
				if self.waitingForAsk:
					assert oid == "A1"
					assert side == "sell"
					assert price == 0.5*(100.00 + 100.10) + 0.5
					assert size == 0.5
				if oid == "B1":
		...
		# Hit 1.5 of the bid at $100, 1.0 of the original order and 0.5 of the strategy's bid.
